...
object HttpRoutes {
    ...
    fun getPlaylistUrl(examId: String, subtenantId: String, playlistTypeId: String): String {
        return "$PLAYLIST$examId/$subtenantId/$playlistTypeId"
    }

    fun getSubjects(examId:String, subtenantId: String, tenantId:String, status:String): String {
        return "$SUBJECTS$examId?tenantIds=$tenantId&subtenantIds=$subtenantId&status=$status"
    }
}